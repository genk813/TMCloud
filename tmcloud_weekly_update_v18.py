...
class WeeklyDataUpdater:
    ...
    def update_intl_goods_services_jp(self, tsv_path):
        """マドプロ指定商品役務の日本語訳を更新"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        # テーブルがなければ作成
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS intl_trademark_goods_services_jp (
            add_del_id TEXT,
            jpo_rfr_num TEXT,
            ...
            PRIMARY KEY (jpo_rfr_num, jpo_rfr_num_split_sign_cd, history_num, seq_num)
        )
        ''')
        ...
        with open(tsv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                primary_keys = (
                    row.get('jpo_rfr_num'),
                    row.get('jpo_rfr_num_split_sign_cd'),
                    row.get('history_num'),
                    int(row.get('seq_num', 0)) if row.get('seq_num') else 0
                )
                
                # 既存データをチェック
                cursor.execute("""
                    SELECT COUNT(*) FROM intl_trademark_goods_services_jp 
                    WHERE jpo_rfr_num = ? AND jpo_rfr_num_split_sign_cd = ? 
                    AND history_num = ? AND seq_num = ?
                """, primary_keys)
                exists = cursor.fetchone()[0] > 0
                
                if exists:
                    cursor.execute("""
                        UPDATE intl_trademark_goods_services_jp 
                        SET add_del_id = ?, madpro_class = ?, goods_service_name = ?,
                            goods_service_japanese_name = ?, force_occur_dt = ?,
                            define_flg = ?, updt_year_month_day = ?, batch_updt_year_month_day = ?
                        WHERE jpo_rfr_num = ? AND jpo_rfr_num_split_sign_cd = ? 
                        AND history_num = ? AND seq_num = ?
                    """, (
                        row.get('add_del_id'),
                        row.get('madopro_class'),
                        row.get('goods_service_name'),
                        row.get('goods_service_japanese_name'),
                        row.get('force_occur_dt'),
                        row.get('define_flg'),
                        row.get('updt_year_month_day'),
                        row.get('batch_updt_year_month_day'),
                        *primary_keys
                    ))
                    updated += 1
                else:
                    cursor.execute("""
                        INSERT INTO intl_trademark_goods_services_jp (
        ...
        print(f"  intl_trademark_goods_services_jp: 新規{inserted}件、更新{updated}件")
        return inserted, updated
    ...
    def update_from_directory(self, tsv_dir):
        ...
        update_files = {
            ...
            'intl_trademark_goods_services_jp': 'upd_dsgn_gvrnmnt_dsgn_st_gds_srvc_app.tsv'
        }
        ...
        for table_name, file_name in update_files.items():
            ...
            if file_path.exists():
                ...
                elif table_name == 'intl_trademark_goods_services_jp':
                    inserted, updated = self.update_intl_goods_services_jp(file_path)
